...
#pragma warning restore SA1629 // Documentation text should end with a period
        ...
        /// </returns>
        /// <remarks>
        /// Setting mitigation policy for a process helps prevent an attacker from exploiting security vulnerabilities. Use the <see cref="SetProcessMitigationPolicy(PROCESS_MITIGATION_POLICY, void*, UIntPtr)"/> function to enable or disable security mitigation programmatically.
        ///
        /// For maximum effectiveness, mitigation policies should be applied before or during process initialization.
        ///
        /// For example, setting the ASLR policy that enables forced relocation of images is effective only if it is applied before all of the images in a process have been loaded. ASLR mitigation policies cannot be made less restrictive after they have been applied.
        /// </remarks>
        [DllImport(nameof(Kernel32), SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static unsafe extern bool SetProcessMitigationPolicy(
            PROCESS_MITIGATION_POLICY MitigationPolicy,
            void* lpBuffer,
            [Friendly(FriendlyFlags.NativeInt)] UIntPtr dwLength);
